...
class Schedule extends MovieAppModel {
	...
	public function get_schedule($data, $parm_date = null, $is_manager = 0, $is_today = true) {
		if ((empty($parm_date) || $is_today) && ($is_manager == 0)) {
	...
	public function get_data_schedule_detail($lang, $movie_id, $movie_type_id, $parm_date = null, $is_manager = 0, $is_today = true) {
		if ((empty($parm_date) || $is_today) && ($is_manager == 0)) {
			$filter_date = date('Y-m-d');
